# 라벨리 웹 — 작업 핸드오프 노트

TASK 2·3·4 자율 실행 세션(2026-08-20)에서 내린 판단, 건너뛴 항목, 성태님 확인이 필요한
사항을 누적 기록합니다.

## TASK 0 — PR #1 머지
- 이전 세션의 TASK 1 PR #1(스캐폴드 + 디자인 이관)을 `--squash`로 머지하고 main 최신화 완료.

## TASK 2 — 앱 실제 화면 반영 + 실제 DB 데이터
브랜치: `feat/task2-real-app-screens`

### 참조한 실제 앱 (oasis-app, 읽기 전용)
- 프로덕션 DB: Supabase `sthzmueifkgfxcixylry` (oasis-app-kr, 서울). 구 싱가포르 프로젝트
  `nzymbktsulhjssbkqhwm`는 사용 안 함(oasis-app CLAUDE.md 확인).
- 결과 화면: `ProductDetailScreen.tsx` / `ScoreCard.tsx` / `NutritionTable.tsx`,
  히스토리: `HistoryScreen.tsx`, 셀프 등록: `SelfRegistrationScreen.tsx`.

### 2-1 화면 미러링
**① 스캔 결과 카드 (홈 히어로 폰 목업)**
- 실제 값으로 교체: 종합건강관리 점수 **48**(실제 47.6 반올림), 순위 **"라면 중 상위 53%"**
  (실제 percentile 53 — 카테고리 유탕면 38개 중 20등), 영양성분 실제 per-100g
  (나트륨 **1,360mg** / 당류 **5g** / 단백질 **8g**) + 실제 앱과 동일하게 **"100g당"** 표기 추가.
- 판단(원칙 D: 임의 리디자인 금지): 실제 앱 결과화면은 점수를 "색 박스 + 숫자 + 한글 등급(보통)
  + 순위줄"과 영양성분 목록으로 보여주지만, 웹 히어로의 링/스탯 타일은 웹 고유 디자인 톤이라
  구조는 바꾸지 않고 **데이터·문구만** 정확히 맞췄습니다. 이미지 출처 캡션은 실제 앱엔 있으나
  데모 신라면은 이미지가 없어(placeholder) 웹 아이콘 플레이스홀더가 오히려 정확 → 추가 안 함.

**② 히스토리 (홈 페르소나 데모 폰)**
- 실제 앱 히스토리 행 구성(썸네일 + 제품명 + 우측 등급색 점수배지)과 배지 위치는 웹 데모가
  이미 일치. 점수는 2-2에서 실제 값으로 교체.
- 판단: 실제 앱 행 메타는 "카테고리 · 시간", 배지는 원형+숫자(점 없음)이고 웹은
  "브랜드·용량"+코멘트 + "N점" 필. 이는 웹 디자인 톤 + 데모 서사(페르소나별 이유 설명)라
  유지. 배지 위치·등급 색 체계는 일치.

**③ 셀프 등록 슬롯**
- 실제 앱은 사진 2장(제품 앞면 + 뒷면/영양정보) → 웹 2슬롯 유지, 두 번째 라벨
  **"표시사항" → "뒷면"**으로 교체(실제 촬영 단계 "제품 뒷면(영양정보)" 반영).

### 2-2 실제 DB 데이터 교체 (SELECT만 사용)
- 데모 제품명(신라면/새우깡/메로나)은 유지. 정확 SKU가 DB에 없어 가장 가까운 실제 제품의
  실제 점수를 사용:
  - **신라면 → "신라면 큰사발"(114g)** — 플레인 신라면 120g 봉지는 visible products에 없음.
  - **새우깡 → "깐풍새우깡"(80g)** — 플레인 새우깡 180g은 영양정보 null이라 persona_scores 없음.
  - **메로나 → "메로나"(75ml)** — 정확 일치.
- 페르소나별 실제 점수(`persona_scores.final_score` 반올림)로 `site-config.ts`의 `DEMO` 교체.
  personas는 `is_filter_type=false` 7종만(비건 제외).
- 히어로 영양성분: 신라면 큰사발 실제 per-100g (나트륨 1359.6→1,360 / 당류 5.3→5 / 단백질 7.9→8).
- **대체상품 검증**: 웹이 쓰던 "오뚜기 진라면 순한맛"은 **DB에 없음** → 저염식단 기준 신라면(0점)보다
  점수 높은 실제 유탕면으로 교체. **농심 사리면** 선택(같은 농심·유탕면, 나트륨 382 vs 신라면
  1,360/100g로 실제로 훨씬 낮음, 저염식단 50 vs 0, 종합 57 vs 48). 히어로(종합건강관리)엔 57점,
  저염식단 대체블록엔 신라면 **0점** → 사리면 **50점**으로 반영.

### 데이터 주의 (성태님 확인 권장)
- **유탕면 카테고리에 선혜청 전통한과(오복/고향생각/만수무강 등)** 여러 건이 저염식단 100·나트륨
  0으로 섞여 있음 = 카테고리 분류 오류로 보임(한과가 유탕면으로 분류). 대체상품엔 명백한 실제
  라면(농심 사리면)만 써서 회피했으나, **앱 자체의 대체상품 추천 품질에도 영향** 갈 수 있으니
  카테고리 정합성 점검 권장.
- 신라면 저염식단 점수 **0.0**은 실제 값(초고나트륨 → 0 하한). 데모에 그대로 노출.

## TASK 3 — 통계 자동 갱신 (GitHub Actions)
브랜치: `feat/task3-stats-automation`

- **`.github/workflows/update-stats.yml`**: `schedule: cron '0 15 * * *'`(UTC 15:00 = KST 익일 00:00)
  + `workflow_dispatch`. 통계가 바뀐 경우에만 커밋 + 재배포.
- **`scripts/update-stats.mjs`**: Supabase REST(PostgREST)로 COUNT만 조회(HEAD + `Prefer: count=exact`).
  - 상품 수 = `products?visible_to_users=is.true`, 페르소나 수 = `personas?is_filter_type=is.false`
    (안 걸면 비건 포함 8로 잘못 표기). 알레르기 21종은 조회 안 하고 하드코딩 유지.
  - `public/stats.json`에 `{ productCount, personaCount, updatedAt }` 기록, **개수가 바뀐 경우에만** 커밋.
- **재배포(3-4)**: GITHUB_TOKEN 커밋은 다른 워크플로를 트리거하지 않으므로, 통계가 바뀌면 같은
  워크플로의 `deploy` 잡에서 빌드 + Pages 배포. deploy 워크플로(TASK 4)와 `concurrency: group=pages`
  공유로 직렬화. **Pages 활성화 자체는 TASK 4에서** 하므로, 그 전에 이 워크플로가 수동 실행되면
  배포 단계는 실패할 수 있음(크론 주기상 실제 문제 가능성 없음).
- **프론트(3-6)**: `src/lib/useSiteStats.ts`가 런타임에 `/stats.json` fetch → 실패/부재 시
  `site-config.ts`의 `defaultStats` 폴백(숫자 안 비어 보임). 홈 신뢰 바 카운트업 유지, 소개 페이지는
  값만 표시. 표시 규칙 `floor(n/100)*100` + 콤마 + `"+"` 유지. 초기 `stats.json`은 실제 현재 값
  (productCount **5849** visible, personaCount **7**).
- **버그 수정(원칙 D: 버그 수정 OK)**: 최초 구현에서 신뢰 바 카운트업이 `stats.json`이 카운트업
  도중 도착하면 **프리즈**하는 문제를 발견·수정 — 카운트업을 "가시성 플래그 + 현재값→목표값 재타겟"
  구조로 바꿔, fetch가 언제 끝나도 목표값으로 매끄럽게 이어지게 함. (헤드리스 브라우저로 확인)

## TASK 4 — 배포 + SEO
브랜치: `feat/task4-deploy-seo`

- **4-1 배포**: `.github/workflows/deploy.yml`(`push: main` + `workflow_dispatch`) → build → GitHub Pages 배포.
  Pages는 `gh api`로 활성화 완료(`build_type=workflow`).
- **4-2 커스텀 도메인**: `public/CNAME = labelly-app.com`, Vite `base: '/'`. 도메인 소유권(TXT)은 완료 상태.
  - ⚠️ **DNS A 레코드 미설정**이라 배포돼도 `labelly-app.com` 접속은 아직 안 됨(정상, 오류 아님).
    CNAME 설정 시 기본 URL(`wkbgff.github.io/labelly-web`)은 커스텀 도메인으로 리다이렉트되므로 DNS 전엔
    어느 URL로도 라이브 확인이 안 됨 → 로컬 정적서버(Pages 동작 모사)로 대체 검증함.
- **4-3 SPA 폴백**: `vite.config.ts`가 빌드 후 `dist/index.html → dist/404.html` 복사. Pages가 알 수 없는
  경로에 404.html을 주면 같은 SPA가 부팅돼 React Router가 렌더. Pages 동작(404→404.html, 상태 404)을
  모사한 정적 서버로 **`/delete-account`·`/about`·`/support` 직접 접근이 모두 정상 렌더**됨을 확인
  (알 수 없는 경로는 홈으로). ⚠️ 딥링크 HTTP 상태는 **404**(페이지는 정상 렌더) — 사람/심사자에겐
  무관하나 SEO 인덱싱엔 불리. **TASK 5 프리렌더로 200 전환 가능**(선택).
- **4-4 SEO**: 페이지별 title/description(`useDocumentMeta`, TASK 1)에 더해 `index.html`에 OG/Twitter 태그
  + `theme-color` 추가. `public/robots.txt`, `public/sitemap.xml`(4개 경로) 추가. 홈 title은 요구대로
  검색어 기준("라벨리 — 한국 식품 바코드 스캔, 목표별 건강점수"), description에 바코드 스캔·식품 성분·
  알레르기·건강점수 키워드 포함. OG 이미지는 **자리표시자**(`https://labelly-app.com/og-image.png`).

## TASK 5 — 프리렌더 (선택) — **건너뜀**
브랜치: `feat/task5-prerender` (문서 기록만, 코드 변경 없음)

TASK 1에서 지적한 no-JS 크롤러 이슈를 프리렌더로 해소하는 선택 과제. 태스크의 엄격한 조건
("시각적 출력 1px도 불변 / 빌드 불안정 시 즉시 되돌리고 건너뜀 / 오래 걸리면 시도하지 말 것 /
안 해도 됨")과 충돌해 **건너뛰기로 결정**:

- **1px 불변 충족 어려움**: 홈(카운트업·히어로 스캔 사이클·스티키 스크롤 데모)과 소개(스크롤 스태거
  reveal — 등장 전 opacity 0)는 프리렌더된 상태 HTML과 클라이언트 초기 상태가 달라, `createRoot`
  재렌더 시 깜빡임(콘텐츠가 보였다 사라졌다 다시 등장)이 발생할 위험이 큼.
- **빌드 안정성/시간**: 프리렌더(react-snap 또는 Playwright 기반)를 CI에 넣으면 Chromium 다운로드 등으로
  빌드가 무거워지고 불안정해질 수 있음 → 조건 위반. (로컬 전용 프리렌더는 CI 재빌드 배포에 반영 안 됨.)
- **핵심 요건은 이미 충족**: 회원탈퇴 아코디언 접힌 콘텐츠는 항상 DOM에 있고(조건부 렌더링 없음),
  딥링크는 404.html 폴백으로 정상 렌더됨. 남는 건 no-JS 크롤러 미대응인데, 이는 선택/스킵 허용 범위.
- **권장**: 나중에 필요하면 깜빡임을 피하는 방식(초기 상태로 프리렌더 + `hydrateRoot` 사용, 또는
  애니메이션 없는 정적 페이지만 대상)으로 별도 집중 작업 권장.

## 성태님이 직접 확인/처리해야 할 것 (누적)
- **(TASK 4) DNS A 레코드 설정**(성태님 직접) — 설정돼야 `labelly-app.com`으로 접속 가능. 그 전까지는
  배포는 되지만 커스텀 도메인 접속 불가(정상).
- **(TASK 4) OG 이미지 `public/og-image.png`(1200x630) 추가** — 현재 링크만 있고 이미지 없음(자리표시자).
- **(TASK 3) GitHub 저장소 Settings → Secrets and variables → Actions 에 시크릿 2개 등록**
  (값은 성태님이 직접 — 저는 이름만 참조):
  - `SUPABASE_URL` (예: `https://sthzmueifkgfxcixylry.supabase.co`)
  - `SUPABASE_PUBLISHABLE_KEY` (`sb_publishable_...` — publishable key, 권한 최소화)
  - 등록 전에는 통계 자동 갱신이 **스킵**되고(워크플로 안내 후 정상 종료), 사이트는 기본값
    (5,800+ / 21종 / 7가지)으로 정상 동작.
- (TASK 2) 위 "데이터 주의": 유탕면 카테고리에 한과 오분류 — 확인 권장.
- (TASK 2 판단) 데모 SKU 대체(신라면→큰사발, 새우깡→깐풍새우깡)와 대체상품(진라면→농심 사리면)
  교체가 마케팅상 괜찮은지 확인.
